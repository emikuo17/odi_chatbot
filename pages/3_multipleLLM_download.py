# STREAMLIT APP with RAG EXTENSION + PA6 Batch Runner (3 LLMs)
# ✅ Keeps your original RAG pipeline + structured prompt controls + chat UI
# ✅ Adds: paste 10 questions -> run 3 LLMs (GPT/Claude/Gemini) -> progress bar -> results table -> download Excel
# ✅ Streamlit Cloud safe: SentenceTransformer lazy-load + CPU
# ✅ Excel export uses openpyxl (make sure it's in requirements.txt)
...
# -----------------------
# PA6 Batch Runner Helpers (3 LLMs, same prompt + same RAG context per question)
# -----------------------
BATCH_MODELS = {
...
def safe_call_one(api_key: str, model_id: str, sys_prompt: str, question: str,
                  temperature: float = 0.2, max_tokens: int = 600) -> str:
    try:
...
def run_batch_eval(api_key: str,
                   task: str, persona: str, tone: str, data_rules: str, scope: str,
                   pref_schema: str, mapping_guide: str, workflow: str, output_rules: str,
                   questions: List[str],
                   top_k: int = 5,
                   temperature: float = 0.2,
                   max_tokens: int = 600) -> pd.DataFrame:
    rows = []
    total_calls = max(1, len(questions) * len(BATCH_MODELS))
    done = 0
    prog = st.progress(0)
    ...
    for q in questions:
        ...
        # 3) Call all 3 models
        for label, model_id in BATCH_MODELS.items():
...
# -----------------------
st.divider()
st.header("📊 PA6 Batch Evaluation (Run 10 Questions × 3 LLMs)")
...
with colB:
    ...
    include_context_col = st.checkbox("Include RAG_Context column in Excel", value=True)

questions = [q.strip() for q in questions_text.splitlines() if q.strip()]

run_disabled = (not api_key) or (len(questions) == 0)
if run_disabled:
    st.caption("To run batch: enter API key + paste at least 1 question. Also upload & embed CSVs for real RAG results.")

if st.button("🚀 Run Batch (3 LLMs)", disabled=run_disabled, use_container_width=True):
    if st.session_state.vector_db is None or not st.session_state.vector_db_ready:
        st.warning("RAG is not ready. Please upload CSVs and click 'Load & Embed CSVs' first.")
    else:
        with st.spinner("Running batch… this will call 3 models for each question."):
            df = run_batch_eval(
                api_key=api_key,
