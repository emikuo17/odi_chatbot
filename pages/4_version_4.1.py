...
# ✅ Minimal change (Emily): _row_to_product_card() now serializes each product row
#    as a structured JSON string instead of hand-built text lines.
#    This improves LLM retrieval because key-value structure is preserved unambiguously.
#    Everything else (ChromaDB, RAG pipeline, batch runner, prompts) is UNCHANGED.
...
# ✅ MINIMAL CHANGE (Emily): Replaced hand-built text lines with json.dumps().
#    Before: manually concatenated "Field: Value\n" strings.
#    After:  serialize the row as a clean JSON object so the LLM receives
#            structured key-value pairs — easier to match fields precisely.
#    All other logic (which rows to skip, how IDs are built, etc.) is unchanged.
def _row_to_product_card(row: pd.Series) -> str:
    # Build a dict from the row, dropping empty/blank values
    row_dict = {}
    ...
    # Serialize as indented JSON so the LLM receives a structured product card
