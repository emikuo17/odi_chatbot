...
Rules:
- You MUST ask at most ONE follow-up question.
- Keep it short: 1–2 sentences.
- The follow-up should be something a real rider would ask (fit, comfort, thickness, lock-on, hand numbness, riding type, color).
- If the assistant already answered clearly, ask no follow-up and reply exactly: "NO_FOLLOWUP"
- Do NOT mention you are an AI or a bot.
- Do NOT request competitor brands by name; stay brand-neutral (the assistant will restrict to ODI).
"""


def build_customer_followup_prompt(seed_question: str, assistant_answer: str) -> str:
    return f"""Seed customer question:
{seed_question}
...
Task:
Generate ONE realistic follow-up question to clarify or confirm something, OR output exactly "NO_FOLLOWUP" if no follow-up is needed.
"""
...
def safe_customer_followup(
    ...
    try:
        user_prompt = build_customer_followup_prompt(seed_question, assistant_answer)
...
def run_conversation_simulation(
    ...
    customer_temp: float = 0.7,
    customer_max_tokens: int = 120,
        ...
        for label, assistant_model_id in model_items:
            ...
            if enable_followup and not str(assistant_answer_1).startswith("ERROR:"):
                followup_q = safe_customer_followup(
                    ...
                    assistant_answer=assistant_answer_1,
                    customer_system=customer_system,
                    temperature=customer_temp,
                ...
                if followup_q.strip().upper() != "NO_FOLLOWUP" and not followup_q.startswith("ERROR:"):
                    ...
                    transcript.append({"role": "assistant", "content": assistant_answer_2})
            ...
            rows.append({
                ...
                "customer_model": customer_model_id,
                "seed_question": seed_q,
...
with sim_left:
    sim_questions_text = st.text_area(
        "Paste Reddit-style seed questions (one per line). These are the conversation starters.",
        height=200,
        placeholder="My hands go numb after rocky trails...\nI have large hands and need thicker grips...\n..."
    )
...
with sim_right:
    ...
    customer_choice = st.selectbox(
        ...
        index=0,
        key="customer_bot_model_choice",
    )
    if MODEL_PRESETS[customer_choice] == "__custom__":
        customer_model = st.text_input("Customer Bot Model (OpenRouter ID)", value="openai/gpt-4.1-mini", key="customer_custom_id")
    else:
        customer_model = MODEL_PRESETS[customer_choice]
        st.caption(f"Customer model: `{customer_model}`")

    sim_assistant_temp = st.slider("Assistant temperature", 0.0, 1.0, 0.2, 0.05, key="sim_assistant_temp")
    sim_assistant_max_tokens = st.number_input("Assistant max_tokens", 100, 2000, 600, 50, key="sim_assistant_max_tokens")
    sim_customer_temp = st.slider("Customer temperature", 0.0, 1.0, 0.7, 0.05, key="sim_customer_temp")
...
sim_run_disabled = (not api_key) or (len(sim_seed_questions) == 0) or (len(sim_assistant_models) == 0)
if sim_run_disabled:
    st.caption("To run simulation: enter API key + paste at least 1 seed question + select at least 1 assistant model. Also upload & embed CSVs for RAG.")

if st.button("🧪 Run Conversation Simulation", disabled=sim_run_disabled, use_container_width=True):
    if st.session_state.vector_db is None or not st.session_state.vector_db_ready:
        st.warning("RAG is not ready. Please upload CSVs and click 'Load & Embed CSVs' first.")
    else:
        with st.spinner("Running simulation… (Customer Bot + Assistant Bot)"):
            sim_df = run_conversation_simulation(
                ...
                customer_system=customer_system,
                top_k=int(top_k),
                assistant_temp=float(sim_assistant_temp),
                assistant_max_tokens=int(sim_assistant_max_tokens),
                customer_temp=float(sim_customer_temp),
                customer_max_tokens=int(sim_customer_max_tokens),
                enable_followup=bool(sim_enable_followup),
            )

        if not sim_include_context_cols:
            drop_cols = [c for c in ["rag_context_seed", "rag_context_followup"] if c in sim_df.columns]
            if drop_cols:
...
if st.session_state.sim_df is not None:
    ...
    sim_csv_bytes = st.session_state.sim_df.to_csv(index=False).encode("utf-8")
    st.download_button(
    ...
    sim_json_bytes = json.dumps(
        st.session_state.sim_df.to_dict(orient="records"),
        indent=2,
        ensure_ascii=False
    ).encode("utf-8")
